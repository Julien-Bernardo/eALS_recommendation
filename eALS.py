...
class eALS():
        ...
        for f in range(self.factors):
            ...
            for u in user_list:
                ...
                numer += (w_users[u] * rating_users[u] - (w_users[u] - Wi[i]) * prediction_users[u]) * U[
                    u, f]  # eq.[13], first part of numerator
    ...
    def fit_rdd(self, max_iterations, sc):
        '''
        ...
        SU_update = sc.broadcast(SU)
        SV_update = sc.broadcast(SV)
        ...
        for iteration in range(max_iterations):
            ...
            U = sc.parallelize(range(self.nb_users)).map(
                lambda u: self.update_user(u, W, Wi, U_update, V_update, SU_update, SV_update, prediction_items,
                                           rating_items, w_items)).collect()
            U_update = np.array(U)

            SU = U_update.T @ U_update
            SU_update = sc.broadcast(SU)

            # build a RDD for items and update items matrix in parallel
            V = sc.parallelize(range(self.nb_items)).map(
                lambda i: self.update_item(i, W, Wi, U_update, V_update, SU_update, SV_update, prediction_users,
                                           rating_users, w_users)).collect()
            V_update = np.array(V)
            ...
            for f in range(self.factors):
                for k in range(f):
                    val = 0
                    for i in range(self.nb_items):
                        val += V_update[i, f] * V_update[i, k] * Wi[i]
                    SV[f, k] = val
                    SV[k, f] = val
            ...
            #current_loss = self.loss(U_update, V_update, W, Wi, SV_update.value)
            # if iteration%5 == 0:
            #print('Iteration:', iteration + 1, 'Loss:', current_loss)

        return U_update, V_update

    def fit_df(self, max_iterations, sc, spark, sqlContext):
        # initialization of parameters
        W, Wi = self.weight_init()
        prediction_items = np.zeros(self.nb_items)
        rating_items = np.zeros(self.nb_items)
        ...
        for iteration in range(max_iterations):
            df_update_user = udf(
                lambda u: self.update_user(u, W, Wi, U_update, V_update, SU_update, SV_update, prediction_items,
                                           rating_items, w_items), ArrayType(FloatType()))
